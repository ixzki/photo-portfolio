...
- 时间支持 Unix 秒、Unix 毫秒及 `2024-05-01T08:00:00+08:00` 等 ISO 格式。没有时区的时间固定按北京时间 UTC+8 处理，不受电脑时区影响。
...
仓库提供 [example-journey.csv](../examples/example-journey.csv)：49 个合成点，时间为北京时间 `2024-05-01 08:00` 至 `2024-05-02 10:50`，包含海拔与定位误差。使用默认断段阈值和 100 米精度筛选时得到 2 段轨迹，与内置示例逐点一致。

`test/travel-route.test.mjs` 会验证自动识别时间、导入坐标与元数据、断段和封面统计。此文件不包含真实足迹，配图也不对应这些坐标。见[示例数据说明](../examples/README.md)。
