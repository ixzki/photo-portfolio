...
选择文件后会先扫描有效时间、经纬度记录，按最早和最晚时间自动填入北京时间范围，不依赖 CSV 行顺序或电脑时区。该扫描不受定位误差筛选影响，也不加载完整轨迹到内存，因此可先识别大型完整导出文件，再手动缩小到本次旅行。小数秒边界向外取整到秒，避免漏掉首尾记录；仅有一个时间会提示无法生成连续行程。更换文件会重新识别，手动调整后点击“读取 CSV”沿用所选范围；“重新识别时间”恢复文件范围。
