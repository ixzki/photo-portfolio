...
- **旅行地图**：整屏展示不同灰度的旅行路线，点击进入游记；封面后展示全屏地图，随滚动绘制完整行程，并显示精选点位的名称、时间与海拔。开启减少动态效果时直接展示完整路线。
- **游记阅读**：正文与图片居中，照片保持比例并限制在视口高度内；左侧地图跟随阅读位置，右侧点位列表以字号、字重和间距动画提示当前段落，支持点击跳转。手机端使用紧凑的地图和导航。
- **旅行后台**：导入“一生足迹”等 CSV，保留时间和海拔；筛选时间范围，在地图上添加点位、选取路段或手绘线路，为每段行程编写 Markdown 图文，从正文点位中选择全程地图的精选点位，支持草稿与发布。
