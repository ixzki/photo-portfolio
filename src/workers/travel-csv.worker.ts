import { detectTravelCsvTimeRange, importTravelCsvFile } from "../lib/travel-csv";
...
const context = self as unknown as {
  onmessage: (event: MessageEvent<{ file: File; task?: "range" | "import"; options: TravelCsvOptions }>) => void;
...
context.onmessage = async ({ data }) => {
  try {
    ...
    if (data.task === "range") {
      const range = await detectTravelCsvTimeRange(data.file, onProgress);
      context.postMessage({ type: "range", ...range });
      return;
    }
    const result = await importTravelCsvFile(data.file, data.options, onProgress);
