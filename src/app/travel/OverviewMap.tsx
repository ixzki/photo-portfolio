...
import type { PublishedJourneyOverview } from "@/lib/journeys";
import { useSiteLoading } from "@/components/SiteLoading";
import styles from "./travel.module.css";

export default function OverviewMap({ routes }: { routes: PublishedJourneyOverview[] }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(element, { zoomControl: false, scrollWheelZoom: true, zoomSnap: 0.25 }).setView([35, 105], 4);
    mapRef.current = map;
    ...
    let failed = 0;
    let loaded = 0;
    const markReady = () => element.setAttribute("data-page-ready", "true");
    tiles.on("loading", () => { failed = 0; loaded = 0; });
    tiles.on("tileerror", () => { failed++; });
    tiles.on("tileload", () => { loaded++; markReady(); });
    tiles.on("load", () => { markReady(); setMapError(failed > 0 && loaded === 0); });

    const allBounds = L.latLngBounds([]);
    const renderer = L.svg({ padding: 0.2 });
    for (const { slug, shade, title, segments } of routes) {
      const href = `/travel/${encodeURIComponent(slug)}`;
      const navigate = () => { startNavigation(href); router.push(href); };
      const routeBounds = L.latLngBounds(segments.flat());
      if (!routeBounds.isValid()) continue;
      allBounds.extend(routeBounds);
      const lines = segments.map((points) => L.polyline(points, {
      ...
      for (const [index, segment] of segments.entries()) {
        ...
        if (index === 0) {
          ...
          path?.setAttribute("aria-label", `查看${title}路线`);
          path?.addEventListener("focus", () => highlight(true));
          path?.addEventListener("blur", () => highlight(false));
          path?.addEventListener("keydown", (event) => {
            if (event instanceof KeyboardEvent && event.key === "Enter") { event.preventDefault(); navigate(); }
          });
        }
      }
    }
    ...
    return () => { resize.disconnect(); map.remove(); mapRef.current = null; element.removeAttribute("data-page-ready"); };
  }, [routes, router, startNavigation]);
