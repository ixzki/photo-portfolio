...
const draft = () => ({ id: "p1", slug: "test", titleZh: "标题", design: "", city: "", time: "", equipment: "", order: 0, visible: false,
  coverUrl: "https://cdn.example.com/cover.jpg", thumbUrl: "https://cdn.example.com/list.jpg", coverW: 6000, coverH: 4000, thumbW: 600, thumbH: 400,
  rows: [{ id: "r1", order: 6, layout: "half", images: [{ id: "i1", url: "https://cdn.example.com/1.jpg", alt: "最新输入的描述", order: 7, width: 1440, height: 960 }] }] });
...
test("saving rejects unfinished photos with the exact row and image to correct", () => {
  ...
  value.rows[0].images[1].url = "https://cdn.example.com/2.jpg"; value.rows[0].images[1].width = 0;
  assert.throws(() => prepareProjectDraft(value), error => error.imageId === "pending" && /尺寸/.test(error.message));
});

test("save normalizes ordering without rewriting custom hosts, mutating the draft or dropping captions", () => {
  const value = draft(); value.coverUrl = "https://legacy.example.com/cover.jpg";
  const saved = prepareProjectDraft(value);
  assert.equal(saved.coverUrl, "https://legacy.example.com/cover.jpg"); assert.equal(saved.featureUrl, saved.coverUrl);
